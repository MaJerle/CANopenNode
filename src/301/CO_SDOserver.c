...
#if (CO_CONFIG_SDO_SRV) & CO_CONFIG_SDO_SRV_BLOCK
        else if (SDO->state == CO_SDO_ST_UPLOAD_BLK_END_CRSP &&
                 data[0] == 0xA1) {
                                                  ...
                                                  uint32_t COB_IDServerToClient)
{
    ...
    SDO->CANtxBuff =
        CO_CANtxBufferInit(SDO->CANdevTx, /* CAN device */
 ...
 */
static ODR_t OD_write_1201_additional(OD_stream_t *stream,
                                      const void *buf,
                                      OD_size_t count,
                                   ...
                                   uint32_t *errInfo)
{
    ...
    if (OD_1200_SDOsrvPar == NULL) {
        ...
        if (OD_SDOsrvParIdx == OD_H1200_SDO_SERVER_1_PARAM) {
            ...
            ODR_t odRet1 =
                OD_get_u32(OD_1200_SDOsrvPar, 1, &COB_IDClientToServer32, true);
            ODR_t odRet2 =
                OD_get_u32(OD_1200_SDOsrvPar, 2, &COB_IDServerToClient32, true);
            ...
            CanId_ClientToServer =
                ((COB_IDClientToServer32 & 0x80000000) == 0)
                    ? (uint16_t)(COB_IDClientToServer32 & 0x7FF)
                    : 0;
            CanId_ServerToClient =
                ((COB_IDServerToClient32 & 0x80000000) == 0)
                    ? (uint16_t)(COB_IDServerToClient32 & 0x7FF)
                    : 0;
            ...
            ODR_t odRetE =
                OD_extension_init(OD_1200_SDOsrvPar, &SDO->OD_1200_extension);
            if (odRetE != ODR_OK) {
                if (errInfo != NULL) {
                    *errInfo = OD_SDOsrvParIdx;
                }
                                   ...
                                   uint16_t crcClient)
{
    ...
    if (SDO->finished) {
        ...
        if (SDO->sizeInd > 0 && SDO->sizeTran != SDO->sizeInd) {
            *abortCode = (SDO->sizeTran > SDO->sizeInd) ? CO_SDO_AB_DATA_LONG
        ...
        /* Verify if size of data downloaded matches data size in OD. */
            ...
            *abortCode = (SDO->sizeTran > sizeInOd) ? CO_SDO_AB_DATA_LONG
                                                    : CO_SDO_AB_DATA_SHORT;
            SDO->state = CO_SDO_ST_ABORT;
            return false;
    ...
    ODR_t odRet = SDO->OD_IO.write(
        &SDO->OD_IO.stream, SDO->buf, SDO->bufOffsetWr, &countWritten);
                         ...
                         bool_t calculateCrc)
{
    ...
    if (!SDO->finished && countRemain < countMinimum) {
        ...
        CO_LOCK_OD(SDO->CANdevTx);
        ODR_t odRet = SDO->OD_IO.read(
            &SDO->OD_IO.stream, bufShifted, countRdRequest, &countRd);
        CO_UNLOCK_OD(SDO->CANdevTx);
                                     ...
                                     uint32_t *timerNext_us)
{
    ...
    /* CAN data received ******************************************************/
        ...
        if (SDO->state == CO_SDO_ST_IDLE) { /* new SDO communication? */
            ...
            if (abortCode == CO_SDO_AB_NONE) {
                ODR_t odRet;
                SDO->index =
                    ((uint16_t)SDO->CANrxData[2]) << 8 | SDO->CANrxData[1];
                SDO->subIndex = SDO->CANrxData[3];
                odRet = OD_getSub(OD_find(SDO->OD, SDO->index),
                                  SDO->subIndex,
                                  &SDO->OD_IO,
                                  false);
                if (odRet != ODR_OK) {
        ...
        if (SDO->state != CO_SDO_ST_IDLE && SDO->state != CO_SDO_ST_ABORT) {
            ...
            case CO_SDO_ST_DOWNLOAD_INITIATE_REQ: {
                if (SDO->CANrxData[0] & 0x02) {
                    ...
                    /* Get SDO data size (indicated by SDO client or get from
                     * OD) */
                    ...
                    /* If dataType is string, then size of data downloaded may
                     * be shorter as size of OD data buffer. If so, add two zero
                     * bytes to terminate (unicode) string. Shorten also OD data
                     * size, (temporary, send information about EOF into
                     * OD_IO.write) */
                    if ((SDO->OD_IO.stream.attribute & ODA_STR) != 0 &&
                    ...
                    /* Verify if size of data downloaded matches data size in
                       OD. */
                    ...
                    CO_LOCK_OD(SDO->CANdevTx);
                    ODR_t odRet = SDO->OD_IO.write(&SDO->OD_IO.stream,
                                                   buf,
                                                   dataSizeToWrite,
                                                   &countWritten);
                    CO_UNLOCK_OD(SDO->CANdevTx);
                    ...
                    if (SDO->CANrxData[0] & 0x01) {
                        ...
                        if (sizeInOd > 0) {
                            ...
                            /* strings are allowed to be shorter */
                                     ...
                                     (SDO->OD_IO.stream.attribute & ODA_STR) ==
                                         0) {
                                abortCode = CO_SDO_AB_DATA_SHORT;
                                SDO->state = CO_SDO_ST_ABORT;
                                break;
            ...
            case CO_SDO_ST_DOWNLOAD_SEGMENT_REQ: {
                if ((SDO->CANrxData[0] & 0xE0) == 0x00) {
                    ...
                    memcpy(
                        SDO->buf + SDO->bufOffsetWr, &SDO->CANrxData[1], count);
                    ...
                    if (SDO->finished || (CO_CONFIG_SDO_SRV_BUFFER_SIZE -
                                          SDO->bufOffsetWr) < (7 + 2)) {
            ...
            case CO_SDO_ST_UPLOAD_BLK_INITIATE_REQ: {
                ...
                if (SDO->sizeInd > 0 && SDO->CANrxData[5] > 0 &&
                    ...
                    if ((SDO->CANrxData[0] & 0x04) != 0) {
                        ...
                        SDO->block_crc =
                            crc16_ccitt(SDO->buf, SDO->bufOffsetWr, 0);
                    } else {
                    ...
                    if (!SDO->finished &&
                        SDO->bufOffsetWr < SDO->block_blksize * 7U) {
                        abortCode = CO_SDO_AB_DEVICE_INCOMPAT;
                        SDO->state = CO_SDO_ST_ABORT;
                        break;
                    }
            ...
            case CO_SDO_ST_UPLOAD_BLK_SUBBLOCK_CRSP: {
                if (SDO->CANrxData[0] == 0xA2) {
                    ...
                    if (SDO->CANrxData[1] < SDO->block_seqno) {
                        ...
                        OD_size_t cntFailed =
                            SDO->block_seqno - SDO->CANrxData[1];
                    ...
                    if (!readFromOd(
                            SDO, &abortCode, SDO->block_blksize * 7, true))
                        break;
    ...
    if (ret == CO_SDO_RT_waitingResponse) {
        ...
        if (SDO->state == CO_SDO_ST_DOWNLOAD_BLK_SUBBLOCK_REQ) {
            ...
            if (SDO->block_timeoutTimer >= SDO->block_SDOtimeoutTime_us) {
                ...
                /* check again after timeout time elapsed */
                uint32_t diff =
                    SDO->block_SDOtimeoutTime_us - SDO->block_timeoutTimer;
                if (*timerNext_us > diff) {
                    *timerNext_us = diff;
                }
            }
    ...
    if (ret == CO_SDO_RT_waitingResponse) {
        ...
        case CO_SDO_ST_UPLOAD_INITIATE_RSP: {
            ...
            CO_LOCK_OD(SDO->CANdevTx);
            ODR_t odRet = SDO->OD_IO.read(
                &SDO->OD_IO.stream, &SDO->CANtxBuff->data[4], 4, &count);
            CO_UNLOCK_OD(SDO->CANdevTx);
        ...
        case CO_SDO_ST_UPLOAD_SEGMENT_RSP: {
            ...
            memcpy(
                &SDO->CANtxBuff->data[1], SDO->buf + SDO->bufOffsetRd, count);
        ...
        case CO_SDO_ST_DOWNLOAD_BLK_SUBBLOCK_RSP: {
            ...
            if (SDO->finished) {
                ...
                OD_size_t count;
                count =
                    (CO_CONFIG_SDO_SRV_BUFFER_SIZE - 2 - SDO->bufOffsetWr) / 7;
                if (count >= 127) {
                    count = 127;
                    ...
                    count =
                        (CO_CONFIG_SDO_SRV_BUFFER_SIZE - 2 - SDO->bufOffsetWr) /
                        7;
                    if (count >= 127) {
                        count = 127;
                    }
