 ...
 */
static ODR_t OD_write_100C(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_100D(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
                                  ...
                                  uint32_t *timerNext_us)
{
    ...
    /* verify "Life time" timeout and update the timer */
        ...
        if (timeDifference_us < ngs->lifeTimer) {
            ...
            /* error bit is shared with HB consumer */
            CO_errorReport(
                ngs->em, CO_EM_HEARTBEAT_CONSUMER, CO_EMC_HEARTBEAT, 0);
        }
    }
                                                ...
                                                uint16_t guardTime_ms)
{
    ...
    ngm->CANtxBuff = CO_CANtxBufferInit(
        ngm->CANdevTx, ngm->CANdevTxIdx, node->ident, true, 1, 0);
#endif
