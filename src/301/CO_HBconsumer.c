 ...
 */
static ODR_t OD_write_1016(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
    ...
    if (stream == NULL || buf == NULL || stream->subIndex < 1 ||
    ...
    CO_ReturnError_t ret =
        CO_HBconsumer_initEntry(HBcons, stream->subIndex - 1, nodeId, time);
    if (ret != CO_ERROR_NO) {
        ...
        for (uint8_t i = 0; i < HBcons->numberOfMonitoredNodes; i++) {
            ...
            if (monitoredNode->HBstate == CO_HBconsumer_ACTIVE) {
                ...
                if (monitoredNode->timeoutTimer >= monitoredNode->time_us) {
                    ...
                    uint32_t diff =
                        monitoredNode->time_us - monitoredNode->timeoutTimer;
                                 ...
                                 CO_NMT_internalState_t *nmtState)
{
    ...
    if (HBcons == NULL || nmtState == NULL ||
        idx >= HBcons->numberOfMonitoredNodes) {
        return -1;
    }
