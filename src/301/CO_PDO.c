 ...
 */
static ODR_t OD_write_dummy(OD_stream_t *stream,
                            const void *buf,
                            OD_size_t count,
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_dummy(OD_stream_t *stream,
                           void *buf,
                           OD_size_t count,
                           OD_size_t *countRead)
{
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_PDO_mapping(OD_stream_t *stream,
                                  const void *buf,
                                  OD_size_t count,
                                  OD_size_t *countWritten)
{
    ...
    if (stream->subIndex == 0) {
        ...
        ODR_t odRet =
            PDOconfigMap(PDO, val, stream->subIndex - 1, PDO->isRPDO, PDO->OD);
        if (odRet != ODR_OK) {
            return odRet;
        }
    }
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_PDO_commParam(OD_stream_t *stream,
                                   void *buf,
                                   OD_size_t count,
                                   OD_size_t *countRead)
{
...
static void CO_PDO_receive(void *object, void *msg)
{
    ...
    if (PDO->valid) {
        if (DLC >= PDO->dataLength) {
            ...
            memcpy(
                RPDO->CANrxData[bufNo], data, sizeof(RPDO->CANrxData[bufNo]));
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_14xx(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
                              ...
                              uint32_t *errInfo)
{
    ...
    ret = PDO_initMapping(
        PDO, OD, OD_16xx_RPDOMapPar, true, errInfo, &erroneousMap);
    if (ret != CO_ERROR_NO) {
        return ret;
    }
    ...
    if (PDO->valid && NMTisOperational
        ...
        if (RPDO->receiveError > CO_RPDO_RX_ACK) {
            ...
            CO_error(PDO->em,
                     ...
                     PDO->dataLength);
            RPDO->receiveError =
                setError ? CO_RPDO_RX_ACK_ERROR : CO_RPDO_RX_ACK_NO_ERROR;
        }
        ...
        while (CO_FLAG_READ(RPDO->CANrxNew[bufNo])) {
            ...
            for (uint8_t i = 0; i < PDO->mappedObjectsCount; i++) {
                ...
                *dataOffset = 0;
                OD_size_t countWritten;
                OD_IO->write(
                    &OD_IO->stream, dataOD, ODdataLength, &countWritten);
        ...
        if (RPDO->timeoutTime_us > 0) {
            if (rpdoReceived) {
                if (RPDO->timeoutTimer > RPDO->timeoutTime_us) {
                    CO_errorReset(
                        PDO->em, CO_EM_RPDO_TIME_OUT, RPDO->timeoutTimer);
                }
                /* enable monitoring */
                RPDO->timeoutTimer = 1;
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_18xx(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
                              ...
                              uint32_t *errInfo)
{
    ...
    CO_ReturnError_t ret = PDO_initMapping(
        PDO, OD, OD_1Axx_TPDOMapPar, false, errInfo, &erroneousMap);
    if (ret != CO_ERROR_NO) {
        return ret;
    }
        ...
        if (TPDO->transmissionType == CO_PDO_TRANSM_TYPE_SYNC_ACYCLIC ||
            ...
            if (TPDO->eventTime_us != 0) {
                TPDO->eventTimer = (TPDO->eventTimer > timeDifference_us)
                                       ? (TPDO->eventTimer - timeDifference_us)
                                       : 0;
...
#if (CO_CONFIG_PDO) & CO_CONFIG_FLAG_TIMERNEXT
            if (TPDO->sendRequest && timerNext_us != NULL &&
