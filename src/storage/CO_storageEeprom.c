...
static ODR_t storeEeprom(CO_storage_entry_t *entry, CO_CANmodule_t *CANmodule)
{
    ...
    uint16_t crc_read = CO_eeprom_getCrcBlock(
        entry->storageModule, entry->eepromAddr, entry->len);
 ...
 * For more information see file CO_storage.h, CO_storage_entry_t.
 */
static ODR_t restoreEeprom(CO_storage_entry_t *entry, CO_CANmodule_t *CANmodule)
{
    (void)CANmodule;
    bool_t writeOk;
                                       ...
                                       uint32_t *storageInitError)
{
    ...
    size_t signaturesAddress =
        CO_eeprom_getAddr(storageModule, false, sizeof(signatures), &eepromOvf);
    ...
    *storageInitError = 0;
    for (uint8_t i = 0; i < entriesCount; i++) {
        ...
        entry->eepromAddr =
            CO_eeprom_getAddr(storageModule, isAuto, entry->len, &eepromOvf);
        entry->offset = 0;
    ...
    for (uint8_t i = 0; i < storage->entriesCount; i++) {
        ...
        if (saveAll) {
            ...
            for (size_t i = 0; i < entry->len;) {
                ...
                if (CO_eeprom_updateByte(
                        entry->storageModule, dataByteToUpdate, eepromAddr)) {
                    i++;
                }
            }
        } else {
            ...
            uint8_t dataByteToUpdate =
                ((uint8_t *)(entry->addr))[entry->offset];
