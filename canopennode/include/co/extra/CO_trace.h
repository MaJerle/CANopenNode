 ...
 * @{
 * In embedded systems there is often a need to monitor some variables over
 * time. Results are then displayed on graph, similar as in oscilloscope.
 *
...
typedef struct {
    ...
    volatile uint32_t
        writePtr; /**< Location in buffer, which will be next written. */
    volatile uint32_t
        readPtr;            /**< Location in buffer, which will be next read. */
    uint32_t lastTimeStamp; /**< Last time stamp. If zero, then last point
                               contains last timestamp. */
    void *OD_variable;      /**< Pointer to variable, which is monitored */
    const CO_trace_dataType_t
        *dt;               /**< Data type specific function pointers. **/
 ...
 * @param map Map to variable in Object Dictionary, which will be monitored.
 * Same structure as in PDO.
 * @param format Format of the plot. If first bit is 1, above variable is
 * unsigned. For more info see Object Dictionary.
 * @param trigger If different than zero, trigger time is recorded, when
 * variable goes through threshold.
 * @param threshold Used with trigger.
 * @param value Pointer to variable, which will show last value of the variable.
 * @param minValue Pointer to variable, which will show minimum value of the
 * variable.
 * @param maxValue Pointer to variable, which will show maximum value of the
 * variable.
 * @param triggerTime Pointer to variable, which will show last trigger time of
 * the variable.
