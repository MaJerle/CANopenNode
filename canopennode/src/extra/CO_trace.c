...
static int32_t getValueI8(void *OD_variable)
{
    return (int32_t) * ((int8_t *)OD_variable);
}
static int32_t getValueI16(void *OD_variable)
{
    return (int32_t) * ((int16_t *)OD_variable);
}
static int32_t getValueI32(void *OD_variable)
{
    return *((int32_t *)OD_variable);
}
static int32_t getValueU8(void *OD_variable)
{
    return (int32_t) * ((uint8_t *)OD_variable);
}
static int32_t getValueU16(void *OD_variable)
{
    return (int32_t) * ((uint16_t *)OD_variable);
}
static int32_t getValueU32(void *OD_variable)
{
    return *((int32_t *)OD_variable);
}
...
static uint32_t
printPointCsv(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    return snprintf(s, size, "%" PRIu32 ";%" PRId32 "\n", timeStamp, value);
}
static uint32_t
printPointCsvUnsigned(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    return snprintf(
        s, size, "%" PRIu32 ";%" PRIu32 "\n", timeStamp, (uint32_t)value);
}
static uint32_t
printPointBinary(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    if (size < 8)
        return 0;
...
static uint32_t
printPointSvgStart(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    return snprintf(s, size, "M%" PRIu32 ",%" PRId32, timeStamp, value);
}
static uint32_t printPointSvgStartUnsigned(char *s,
                                           uint32_t size,
                                           uint32_t timeStamp,
                                           int32_t value)
{
    return snprintf(
        s, size, "M%" PRIu32 ",%" PRIu32, timeStamp, (uint32_t)value);
}
static uint32_t
printPointSvg(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    return snprintf(s, size, "H%" PRIu32 "V%" PRId32, timeStamp, value);
}
static uint32_t
printPointSvgUnsigned(char *s, uint32_t size, uint32_t timeStamp, int32_t value)
{
    return snprintf(
        s, size, "H%" PRIu32 "V%" PRIu32, timeStamp, (uint32_t)value);
}


/* Collection of function pointers for fast processing based on specific data
 * type. */
/* Rules for the array: There must be groups of six members (I8, I16, I32, U8,
 * U16, U32) in correct order and sequence, so findVariable() finds correct
 * member. */
static const CO_trace_dataType_t dataTypes[] = {
    ...
    {getValueU8,
     printPointSvgStartUnsigned,
     printPointSvgUnsigned,
     printPointSvgUnsigned},
    {getValueU16,
     printPointSvgStartUnsigned,
     printPointSvgUnsigned,
...
static void findVariable(CO_trace_t *trace)
{
    ...
    if (!err && (index != 0 || subIndex != 0)) {
        ...
        if (index >= 0x1000 && entryNo != 0xFFFF &&
            subIndex <= trace->SDO->OD[entryNo].maxSubIndex) {
        ...
        if (ODF_arg->reading) {
            ...
            if (*value == 0) {
                ...
                if (trace->bufferSize == 0) {
                    ...
                    /* set trace->OD_variable and trace->dt, based on 'map' and
                     * 'format' */
                    findVariable(trace);
    ...
    case 5: /* plot */
        if (ODF_arg->reading) {
            ...
            if (trace->bufferSize == 0 || ODF_arg->dataLength < 100) {
                ...
                /* repeat everything, if trace->readPtr was overflowed in
                 * CO_trace_process */
                do {
                    ...
                    if (rp != trace->writePtr) {
                        for (;;) {
                            ...
                            if (rp == trace->writePtr) {
                                /* If there is last time stamp, point will be
                                 * printed at the end */
                                if (t != trace->lastTimeStamp) {
                                    len =
                                        trace->dt->printPoint(s, freeLen, t, v);
                                    s += len;
                                    freeLen -= len;
                                }
                            ...
                            /* if output buffer is full, next data will be sent
                             * later */
                            if (freeLen < 50) {
                   ...
                   uint16_t idx_OD_trace)
{
    ...
    *trace->triggerTime = 0;
    ...
    CO_OD_configure(
        SDO, idx_OD_traceConfig, CO_ODF_traceConfig, (void *)trace, 0, 0);
...
void CO_trace_process(CO_trace_t *trace, uint32_t timestamp)
{
    if (trace->enabled) {
        ...
        if (val != trace->valuePrev) {
            ...
            if ((*trace->trigger & 1) != 0 &&
                trace->valuePrev < *trace->threshold &&
                val >= *trace->threshold) {
                *trace->triggerTime = timestamp;
            }
            if ((*trace->trigger & 2) != 0 &&
                trace->valuePrev < *trace->threshold &&
                val >= *trace->threshold) {
                *trace->triggerTime = timestamp;
            }
