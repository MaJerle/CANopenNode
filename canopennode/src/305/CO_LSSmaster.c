...
static void CO_LSSmaster_receive(void *object, void *msg)
{
    ...
    LSSmaster = (CO_LSSmaster_t *)
        object; /* this is the correct pointer type of the first argument */

    /* verify message length and message overflow (previous message was not
     * processed yet) */
    if (DLC == 8 && !CO_FLAG_READ(LSSmaster->CANrxNew) &&
        ...
        /* Optional signal to RTOS, which can resume task, which handles further
         * processing. */
                                       ...
                                       CO_LSS_address_t *lssAddress)
{
    ...
    if (lssAddress != NULL) {
        ...
        CO_FLAG_CLEAR(LSSmaster->CANrxNew);
        memset(&LSSmaster->TXbuff->data[6],
               0,
               sizeof(LSSmaster->TXbuff->data) - 6);
        LSSmaster->TXbuff->data[0] = CO_LSS_SWITCH_STATE_SEL_VENDOR;
        CO_setUint32(&LSSmaster->TXbuff->data[1],
        ...
        memset(&LSSmaster->TXbuff->data[2],
               0,
               sizeof(LSSmaster->TXbuff->data) - 2);
        CO_CANsend(LSSmaster->CANdevTx, LSSmaster->TXbuff);
 ...
 */
static CO_LSSmaster_return_t
CO_LSSmaster_switchStateSelectWait(CO_LSSmaster_t *LSSmaster,
                                   uint32_t timeDifference_us)
{
    CO_LSSmaster_return_t ret;
...
static CO_LSSmaster_return_t CO_LSSmaster_configureCheckWait(
    CO_LSSmaster_t *LSSmaster, uint32_t timeDifference_us, uint8_t csWait)
{
    CO_LSSmaster_return_t ret;
    ...
    CO_LSSmaster_t *LSSmaster, uint32_t timeDifference_us, uint16_t bit)
{
    ...
    switch (bit) {
    case 1000:
        bitTiming = CO_LSS_BIT_TIMING_1000;
        break;
    case 800:
        bitTiming = CO_LSS_BIT_TIMING_800;
        break;
    case 500:
        bitTiming = CO_LSS_BIT_TIMING_500;
        break;
    case 250:
        bitTiming = CO_LSS_BIT_TIMING_250;
        break;
    case 125:
        bitTiming = CO_LSS_BIT_TIMING_125;
        break;
    case 50:
        bitTiming = CO_LSS_BIT_TIMING_50;
        break;
    case 20:
        bitTiming = CO_LSS_BIT_TIMING_20;
        break;
    case 10:
        bitTiming = CO_LSS_BIT_TIMING_10;
        break;
    case 0:
        bitTiming = CO_LSS_BIT_TIMING_AUTO;
        break;
    default:
        return CO_LSSmaster_ILLEGAL_ARGUMENT;
    }
    ...
    if (LSSmaster->state == CO_LSSmaster_STATE_CFG_SLECTIVE &&
        ...
        memset(&LSSmaster->TXbuff->data[3],
               0,
               sizeof(LSSmaster->TXbuff->data) - 3);
        CO_CANsend(LSSmaster->CANdevTx, LSSmaster->TXbuff);
    ...
    /* Wait for confirmation */
        ...
        ret = CO_LSSmaster_configureCheckWait(
            LSSmaster, timeDifference_us, CO_LSS_CFG_BIT_TIMING);
    }
                                                   ...
                                                   uint8_t nodeId)
{
    ...
    if ((LSSmaster->state == CO_LSSmaster_STATE_CFG_SLECTIVE ||
         /* Let un-config node ID also be run in global mode for unconfiguring
            all nodes */
         (LSSmaster->state == CO_LSSmaster_STATE_CFG_GLOBAL &&
    ...
    /* Wait for confirmation */
        ...
        ret = CO_LSSmaster_configureCheckWait(
            LSSmaster, timeDifference_us, CO_LSS_CFG_NODE_ID);
    }
                                                  ...
                                                  uint32_t timeDifference_us)
{
    ...
    if (LSSmaster->state == CO_LSSmaster_STATE_CFG_SLECTIVE &&
        ...
        memset(&LSSmaster->TXbuff->data[1],
               0,
               sizeof(LSSmaster->TXbuff->data) - 1);
        CO_CANsend(LSSmaster->CANdevTx, LSSmaster->TXbuff);
    ...
    /* Wait for confirmation */
        ...
        ret = CO_LSSmaster_configureCheckWait(
            LSSmaster, timeDifference_us, CO_LSS_CFG_STORE);
    }
                               ...
                               CO_LSS_address_t *lssAddress)
{
    ...
    if (LSSmaster->command == CO_LSSmaster_COMMAND_INQUIRE_VENDOR) {
        ...
        ret =
            CO_LSSmaster_inquireCheckWait(LSSmaster,
                                          timeDifference_us,
                                          CO_LSS_INQUIRE_REV,
                                          &lssAddress->identity.revisionNumber);
        if (ret == CO_LSSmaster_OK) {
    ...
    if (LSSmaster->state == CO_LSSmaster_STATE_CFG_SLECTIVE ||
        ...
        if (LSSmaster->command == CO_LSSmaster_COMMAND_WAITING) {
            ...
            ret =
                CO_LSSmaster_inquireInitiate(LSSmaster, CO_LSS_INQUIRE_VENDOR);
        } else if (next == CO_LSSmaster_COMMAND_INQUIRE_PRODUCT) {
            LSSmaster->command = CO_LSSmaster_COMMAND_INQUIRE_PRODUCT;
            LSSmaster->timeoutTimer = 0;

            ret =
                CO_LSSmaster_inquireInitiate(LSSmaster, CO_LSS_INQUIRE_PRODUCT);
            ...
            ret =
                CO_LSSmaster_inquireInitiate(LSSmaster, CO_LSS_INQUIRE_SERIAL);
        }
    }
                                           ...
                                           uint32_t *value)
{
    ...
    /* Check for reply */
        ...
        ret = CO_LSSmaster_inquireCheckWait(
            LSSmaster, timeDifference_us, lssInquireCs, value);
    }
 ...
 * Helper function - send request
 */
static void CO_LSSmaster_FsSendMsg(CO_LSSmaster_t *LSSmaster,
                                   uint32_t idNumber,
                                   uint8_t bitCheck,
                                   uint8_t lssSub,
                                   uint8_t lssNext)
 ...
 */
static CO_LSSmaster_return_t
CO_LSSmaster_FsScanInitiate(CO_LSSmaster_t *LSSmaster,
                            uint32_t timeDifference_us,
 ...
 */
static CO_LSSmaster_return_t
CO_LSSmaster_FsVerifyInitiate(CO_LSSmaster_t *LSSmaster,
                              uint32_t timeDifference_us,
                              CO_LSSmaster_scantype_t scan,
 ...
 */
static CO_LSS_fastscan_lss_sub_next
                              ...
                              CO_LSSmaster_fastscan_t *fastscan)
{
    ...
    /* evaluate fastscan state machine. The state machine is evaluated as
     * following
    ...
    case CO_LSSmaster_FS_STATE_SCAN:
        ret = CO_LSSmaster_FsScanWait(
            LSSmaster, timeDifference_us, fastscan->scan[LSSmaster->fsLssSub]);
        if (ret == CO_LSSmaster_SCAN_FINISHED) {
            ...
            ret = CO_LSSmaster_FsVerifyInitiate(
                LSSmaster,
                timeDifference_us,
                fastscan->scan[LSSmaster->fsLssSub],
                fastscan->match.addr[LSSmaster->fsLssSub],
                next);
    ...
    case CO_LSSmaster_FS_STATE_VERIFY:
        ...
        if (ret == CO_LSSmaster_SCAN_FINISHED) {
            ...
            if (next == CO_LSS_FASTSCAN_VENDOR_ID) {
                ...
                ret = CO_LSSmaster_FsScanInitiate(
                    LSSmaster, timeDifference_us, fastscan->scan[next], next);
                if (ret == CO_LSSmaster_SCAN_FINISHED) {
