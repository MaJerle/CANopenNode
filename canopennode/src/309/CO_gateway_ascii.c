                              ...
                              uint8_t dummy)
{
    ...
    CO_fifo_init(
        &gtwa->commFifo, &gtwa->commBuf[0], CO_CONFIG_GTWA_COMM_BUF_SIZE + 1);

#if (CO_CONFIG_GTW) & CO_CONFIG_GTW_ASCII_LOG
    CO_fifo_init(
        &gtwa->logFifo, &gtwa->logBuf[0], CO_CONFIG_GTWA_LOG_BUF_SIZE + 1);
...
static const char CO_GTWA_helpString[] =
    ...
    "[[<net>] <node>] r[ead] <index> <subindex> [<datatype>]        # SDO "
    "upload.\n"
    "[[<net>] <node>] w[rite] <index> <subindex> <datatype> <value> # SDO "
    "download.\n"
    "\n"
    "[[<net>] <node>] start                   # NMT Start node.\n"
    "[[<net>] <node>] stop                    # NMT Stop node.\n"
    "[[<net>] <node>] preop[erational]        # NMT Set node to "
    "pre-operational.\n"
    ...
    "[<net>] set sdo_timeout <value>          # Configure SDO client time-out "
    "in ms.\n"
    "[<net>] set sdo_block <0|1>              # Enable/disable SDO block "
    "transfer.\n"
    "\n"
    "help [datatype|lss]                      # Print this or datatype or lss "
    "help.\n"
    "led                                      # Print status LEDs of this "
    "device.\n"
    ...
    "Same\n"
    "  is response. String is not null terminated, <CR> is optional in "
    "command.\n"
    "* Comments started with '#' are ignored. They may be on the beginning of "
    "the\n"
    "  line or after the command string.\n"
    "* 'sdo_timeout' is in milliseconds, 500 by default. Block transfer is\n"
    "  disabled by default.\n"
    "* If '<net>' or '<node>' is not specified within commands, then value "
    "defined\n"
...
static const char CO_GTWA_helpStringDatatypes[] =
    ...
    "x8, x16, x32, x64  # Unsigned integers, displayed as hexadecimal, "
    "non-standard.\n"
    "r32, r64           # Real numbers.\n"
    "vs                 # Visible string (between double quotes if "
    "multi-word).\n"
    "os, us             # Octet, unicode string, (mime-base64 (RFC2045) based, "
    "line).\n"
    "d                  # domain (mime-base64 (RFC2045) based, one line).\n"
    "hex                # Hexagonal data, optionally space separated, "
    "non-standard.\r\n";
...
static const char CO_GTWA_helpStringLss[] =
    ...
    "_lss_fastscan [<timeout_ms>]           # Identify fastscan, "
    "non-standard.\n"
    ...
    "                                       # Node-ID configuration of all "
    "nodes.\n"
    "\n"
    "* All LSS commands start with '\"[\"<sequence>\"]\" [<net>]'.\n"
    "* <table_index>: 0=1000 kbit/s, 1=800 kbit/s, 2=500 kbit/s, 3=250 "
    "kbit/s,\n"
    "                 4=125 kbit/s, 6=50 kbit/s, 7=20 kbit/s, 8=10 kbit/s, "
    "9=auto\n"
...
static void responseLSS(CO_GTWA_t *gtwa, CO_LSSmaster_return_t lss_ret)
{
    if (lss_ret == CO_LSSmaster_OK) {
        ...
        if (lss_ret == CO_LSSmaster_TIMEOUT ||
            lss_ret == CO_LSSmaster_SCAN_NOACK) {
                     ...
                     uint32_t *timerNext_us)
{
    ...
    while (gtwa->state == CO_GTWA_ST_IDLE &&
        ...
        for (i = 0; i < 3; i++) {
            closed = -1;
            n = CO_fifo_readToken(
                &gtwa->commFifo, tok, sizeof(tok), &closed, &err);
            if (err || n == 0) {
        ...
        if (strcmp(tok, "set") == 0) {
            ...
            if (strcmp(tok, "network") == 0) {
                ...
                CO_fifo_readToken(
                    &gtwa->commFifo, tok, sizeof(tok), &closed, &err);
                value = (uint16_t)getU32(
                    tok, CO_CONFIG_GTW_NET_MIN, CO_CONFIG_GTW_NET_MAX, &err);
                if (err)
                    break;
        ...
        /* Upload SDO command - 'r[ead] <index> <subindex> <datatype>' */
            ...
            /* initiate upload */
            SDO_ret = CO_SDOclientUploadInitiate(gtwa->SDO_C,
                                                 idx,
                                                 subidx,
                                                 gtwa->SDOtimeoutTime,
        ...
        /* Download SDO comm. - w[rite] <index> <subindex> <datatype> <value> */
            ...
            /* initiate download */
            SDO_ret =
                CO_SDOclientDownloadInitiate(gtwa->SDO_C,
                                             idx,
                                             subidx,
            ...
            size = gtwa->SDOdataType->dataTypeScan(
                &gtwa->SDO_C->bufFifo, &gtwa->commFifo, &status);
         ...
         *   4=125 kbit/s, 6=50 kbit/s, 7=20 kbit/s, 8=10 kbit/s, 9=auto */
            ...
            int maxIndex = (sizeof(CO_LSS_bitTimingTableLookup) /
                            sizeof(CO_LSS_bitTimingTableLookup[0])) -
                           1;
        ...
        /* Inquire LSS address command - 'lss_inquire_addr [<LSSSUB=0..3>]' */
            ...
            if (closed == 0) {
                ...
                switch (lsssub) {
                case 0:
                    gtwa->lssInquireCs = CO_LSS_INQUIRE_VENDOR;
                    break;
                case 1:
                    gtwa->lssInquireCs = CO_LSS_INQUIRE_PRODUCT;
                    break;
                case 2:
                    gtwa->lssInquireCs = CO_LSS_INQUIRE_REV;
                    break;
                default:
                    gtwa->lssInquireCs = CO_LSS_INQUIRE_SERIAL;
                    break;
                }
         ...
         * <scanType2=0..2> <revisionNo> <scanType3=0..2> <serialNo>]]' */
            ...
            if (closed == 0) {
                ...
                CO_fifo_readToken(
                    &gtwa->commFifo, tok, sizeof(tok), &closed, &err);
                fs->match.identity.productCode =
                    getU32(tok, 0, 0xFFFFFFFF, &err);
                if (err)
                    break;
                ...
                CO_fifo_readToken(
                    &gtwa->commFifo, tok, sizeof(tok), &closed, &err);
                fs->match.identity.revisionNumber =
                    getU32(tok, 0, 0xFFFFFFFF, &err);
                if (err)
                    break;
                ...
                CO_fifo_readToken(
                    &gtwa->commFifo, tok, sizeof(tok), &closed, &err);
                fs->match.identity.serialNumber =
                    getU32(tok, 0, 0xFFFFFFFF, &err);
                if (err)
                    break;
            }
    ...
    if (err) {
            ...
            /* Response data must be read, partially or whole */
                ...
                do {
                    ...
                    if (respBufTransfer(gtwa) == false) {
                        /* broken communication, send SDO abort and force
                         * finish. */
                        abortCode = CO_SDO_AB_DATA_TRANSF;
                        CO_SDOclientUpload(
                            gtwa->SDO_C, 0, true, &abortCode, NULL, NULL, NULL);
                        gtwa->state = CO_GTWA_ST_IDLE;
                        break;
                    }
        ...
        case CO_GTWA_ST_WRITE_ABORTED: {
            ...
            if (gtwa->SDOdataCopyStatus) {
                ...
                if (gtwa->state == CO_GTWA_ST_WRITE_ABORTED) {
                    /* Stay in this state, until all data transferred via
                     * commFifo will be purged. */
            ...
            /* If not all data were transferred, make sure, there is enough data
             * in SDO buffer, to continue communication. Otherwise wait and
             * check for timeout */
            if (gtwa->SDOdataCopyStatus &&
        ...
        case CO_GTWA_ST_LSS_SWITCH_GLOB: {
            CO_LSSmaster_return_t ret;
            ret = CO_LSSmaster_switchStateSelect(
                gtwa->LSSmaster, timeDifference_us, NULL);
        ...
        case CO_GTWA_ST_LSS_SWITCH_SEL: {
            CO_LSSmaster_return_t ret;
            ret = CO_LSSmaster_switchStateSelect(
                gtwa->LSSmaster, timeDifference_us, &gtwa->lssAddress);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                responseLSS(gtwa, ret);
                gtwa->state = CO_GTWA_ST_IDLE;
            }
        ...
        case CO_GTWA_ST_LSS_SET_NODE: {
            CO_LSSmaster_return_t ret;
            ret = CO_LSSmaster_configureNodeId(
                gtwa->LSSmaster, timeDifference_us, gtwa->lssNID);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
        ...
        case CO_GTWA_ST_LSS_CONF_BITRATE: {
            CO_LSSmaster_return_t ret;
            ret = CO_LSSmaster_configureBitTiming(
                gtwa->LSSmaster, timeDifference_us, gtwa->lssBitrate);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                if (ret == CO_LSSmaster_OK_ILLEGAL_ARGUMENT) {
        ...
        case CO_GTWA_ST_LSS_STORE: {
            ...
            ret =
                CO_LSSmaster_configureStore(gtwa->LSSmaster, timeDifference_us);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                if (ret == CO_LSSmaster_OK_ILLEGAL_ARGUMENT) {
        ...
        case CO_GTWA_ST_LSS_INQUIRE: {
            ...
            ret = CO_LSSmaster_Inquire(
                gtwa->LSSmaster, timeDifference_us, gtwa->lssInquireCs, &value);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                if (ret == CO_LSSmaster_OK) {
                    if (gtwa->lssInquireCs == CO_LSS_INQUIRE_NODE_ID) {
                        gtwa->respBufCount =
                            snprintf(gtwa->respBuf,
                                     CO_GTWA_RESP_BUF_SIZE,
                                     "[%" PRId32 "] 0x%02" PRIX32 "\r\n",
                                     gtwa->sequence,
                                     value & 0xFF);
        ...
        case CO_GTWA_ST_LSS_INQUIRE_ADDR_ALL: {
            ...
            ret = CO_LSSmaster_InquireLssAddress(
                gtwa->LSSmaster, timeDifference_us, &gtwa->lssAddress);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                if (ret == CO_LSSmaster_OK) {
                    gtwa->respBufCount =
        ...
        case CO_GTWA_ST__LSS_FASTSCAN: {
            ...
            ret = CO_LSSmaster_IdentifyFastscan(
                gtwa->LSSmaster, timeDifference_us, &gtwa->lssFastscan);
            if (ret != CO_LSSmaster_WAIT_SLAVE) {
                if (ret == CO_LSSmaster_OK ||
                    ret == CO_LSSmaster_SCAN_FINISHED) {
                    gtwa->respBufCount = snprintf(
                        gtwa->respBuf,
                        CO_GTWA_RESP_BUF_SIZE,
                        "[%" PRId32 "] 0x%08" PRIX32 " 0x%08" PRIX32
                        " 0x%08" PRIX32 " 0x%08" PRIX32 "\r\n",
        ...
        case CO_GTWA_ST_HELP: {
            ...
            do {
                ...
                size_t lenCopied =
                    lenBuf < lenHelpRemain ? lenBuf : lenHelpRemain;
