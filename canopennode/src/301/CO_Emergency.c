 ...
 */
static ODR_t OD_read_1014(OD_stream_t *stream,
                          void *buf,
                          OD_size_t count,
                          OD_size_t *countRead)
{
...
static ODR_t OD_write_1014(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
    ...
    if ((COB_IDEmergency32 & 0x7FFFF800) != 0 ||
        CO_IS_RESTRICTED_CAN_ID(newCanId) ||
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_1014_default(OD_stream_t *stream,
                                  void *buf,
                                  OD_size_t count,
                                  OD_size_t *countRead)
{
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_1015(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_1003(OD_stream_t *stream,
                          void *buf,
                          OD_size_t count,
                          OD_size_t *countRead)
{
...
static ODR_t OD_write_1003(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_statusBits(OD_stream_t *stream,
                                void *buf,
                                OD_size_t count,
                                OD_size_t *countRead)
{
...
static ODR_t OD_write_statusBits(OD_stream_t *stream,
                                 const void *buf,
                                 OD_size_t count,
                                 OD_size_t *countWritten)
{
                            ...
                            uint32_t *errInfo)
{
...
#endif
#if (CO_CONFIG_EM) & CO_CONFIG_EM_PRODUCER
        || OD_1014_cobIdEm == NULL || CANdevTx == NULL || nodeId < 1 ||
        nodeId > 127
#endif
#if (CO_CONFIG_EM) & CO_CONFIG_EM_HISTORY
    ...
    *em->errorRegister = 0;
    ...
    em->producerEnabled =
        (COB_IDEmergency32 & 0x80000000) == 0 && producerCanId != 0;
    ...
    *em->errorRegister = errorRegister;
    ...
    if (em->fifoSize >= 2) {
        ...
        if (fifoPpPtr != em->fifoWrPtr && !em->CANtxBuff->bufferFull) {
            ...
            memcpy(em->CANtxBuff->data,
                   &em->fifo[fifoPpPtr].msg,
            ...
            if (em->fifoOverflow == 1) {
                em->fifoOverflow = 2;
                CO_errorReport(
                    em, CO_EM_EMERGENCY_BUFFER_FULL, CO_EMC_GENERIC, 0);
            } else if (em->fifoOverflow == 2 &&
                       em->fifoPpPtr == em->fifoWrPtr) {
                em->fifoOverflow = 0;
