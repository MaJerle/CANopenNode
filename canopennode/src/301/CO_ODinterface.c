...
static ODR_t OD_readDisabled(OD_stream_t *stream,
                             void *buf,
                             OD_size_t count,
                             OD_size_t *countRead)
{
...
static ODR_t OD_writeDisabled(OD_stream_t *stream,
                              const void *buf,
                              OD_size_t count,
                              OD_size_t *countWritten)
{
                ...
                bool_t odOrig)
{
    ...
    /* Access data from extension specified by application */
        ...
        io->read = entry->extension->read != NULL ? entry->extension->read
                                                  : OD_readDisabled;
        io->write = entry->extension->write != NULL ? entry->extension->write
                                                    : OD_writeDisabled;
...
uint32_t OD_getSDOabCode(ODR_t returnCode)
{
    ...
    return (returnCode < 0 || returnCode >= ODR_COUNT)
               ? abortCodes[ODR_DEV_INCOMPAT]
               : abortCodes[returnCode];
}
