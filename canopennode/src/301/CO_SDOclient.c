        ...
        if (data[0] == 0x80 /* abort from server */
            ...
            if (seqno <= SDO_C->block_blksize &&
                ...
                if ((data[0] & 0x80) != 0) {
                    ...
                    CO_fifo_write(
                        &SDO_C->bufFifo, &data[1], 7, &SDO_C->block_crc);
                    SDO_C->sizeTran += 7;
 ...
 */
static ODR_t OD_write_1280(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                                   ...
                                   uint32_t *errInfo)
{
    ...
    CO_fifo_init(
        &SDO_C->bufFifo, SDO_C->buf, CO_CONFIG_SDO_CLI_BUFFER_SIZE + 1);
    ...
    ODR_t odRet1 =
        OD_get_u32(OD_1280_SDOcliPar, 1, &COB_IDClientToServer, true);
    ODR_t odRet2 =
        OD_get_u32(OD_1280_SDOcliPar, 2, &COB_IDServerToClient, true);
    ODR_t odRet3 = OD_get_u8(OD_1280_SDOcliPar, 3, &nodeIDOfTheSDOServer, true);

    if (odRet0 != ODR_OK || maxSubIndex != 3 || odRet1 != ODR_OK ||
        odRet2 != ODR_OK || odRet3 != ODR_OK) {
        if (errInfo != NULL)
    ...
    ODR_t odRetE =
        OD_extension_init(OD_1280_SDOcliPar, &SDO_C->OD_1280_extension);
    if (odRetE != ODR_OK) {
        if (errInfo != NULL)
            *errInfo = OD_getIndex(OD_1280_SDOcliPar);
        return CO_ERROR_OD_PARAMETERS;
                                   ...
                                   uint8_t nodeIDOfTheSDOServer)
{
    ...
    uint16_t CanIdC2S = ((COB_IDClientToServer & 0x80000000L) == 0)
                            ? (uint16_t)(COB_IDClientToServer & 0x7FF)
                            : 0;
    uint16_t CanIdS2C = ((COB_IDServerToClient & 0x80000000L) == 0)
                            ? (uint16_t)(COB_IDServerToClient & 0x7FF)
                            : 0;
                                     ...
                                     uint32_t *timerNext_us)
{
    ...
    /* Transfer data locally **************************************************/
        ...
        if (SDO_C->OD_IO.write == NULL) {
            ...
            odRet = OD_getSub(OD_find(SDO_C->OD, SDO_C->index),
                              SDO_C->subIndex,
                              &SDO_C->OD_IO,
                              false);
        ...
        if (SDO_C->OD_IO.write != NULL) {
            ...
            else if (!bufferPartial && SDO_C->sizeInd > 0 &&
            ...
            if (abortCode == CO_SDO_AB_NONE) {
                ...
                ODR_t odRet = SDO_C->OD_IO.write(
                    &SDO_C->OD_IO.stream, buf, (OD_size_t)count, &countWritten);
    ...
    /* CAN data received ******************************************************/
        ...
        if (SDO_C->CANrxData[0] == 0x80) {
            ...
            abortCode = (SDOabortCode != NULL) ? *SDOabortCode
    ...
    if (ret == CO_SDO_RT_waitingResponse) {
        ...
        case CO_SDO_ST_DOWNLOAD_INITIATE_REQ: {
            ...
            if ((SDO_C->sizeInd == 0 && count <= 4) ||
                ...
                if (count == 0 ||
                    (SDO_C->sizeInd > 0 && SDO_C->sizeInd != count)) {
                    SDO_C->state = CO_SDO_ST_IDLE;
                ...
                /* copy data */
                CO_fifo_read(
                    &SDO_C->bufFifo, &SDO_C->CANtxBuff->data[4], count, NULL);
                SDO_C->sizeTran = count;
                SDO_C->finished = true;
            } else {
#if (CO_CONFIG_SDO_CLI) & CO_CONFIG_SDO_CLI_SEGMENTED
        ...
        case CO_SDO_ST_DOWNLOAD_SEGMENT_REQ: {
            /* fill data bytes */
            count = CO_fifo_read(
                &SDO_C->bufFifo, &SDO_C->CANtxBuff->data[1], 7, NULL);
            ...
            SDO_C->CANtxBuff->data[0] =
                (uint8_t)(SDO_C->toggle | ((7 - count) << 1));
        ...
        case CO_SDO_ST_DOWNLOAD_BLK_SUBBLOCK_REQ: {
            ...
            count =
                CO_fifo_altRead(&SDO_C->bufFifo, &SDO_C->CANtxBuff->data[1], 7);
                                   ...
                                   uint32_t *timerNext_us)
{
    ...
    /* Transfer data locally **************************************************/
        ...
        /* read data, in several passes if necessary */
            ...
            CO_LOCK_OD(SDO_C->CANdevTx);
            ODR_t odRet = SDO_C->OD_IO.read(
                &SDO_C->OD_IO.stream, buf, countBuf, &countRd);
            CO_UNLOCK_OD(SDO_C->CANdevTx);
        ...
        if (SDO_C->CANrxData[0] == 0x80) {
            ...
            case CO_SDO_ST_UPLOAD_INITIATE_RSP: {
                if ((SDO_C->CANrxData[0] & 0xF0) == 0x40) {
                    ...
                    if (SDO_C->CANrxData[0] & 0x02) {
                        ...
                        CO_fifo_write(
                            &SDO_C->bufFifo, &SDO_C->CANrxData[4], count, NULL);
                        SDO_C->sizeTran = count;
                        SDO_C->state = CO_SDO_ST_IDLE;
                        ret = CO_SDO_RT_ok_communicationEnd;
                    } else {
            ...
            case CO_SDO_ST_UPLOAD_SEGMENT_RSP: {
                if ((SDO_C->CANrxData[0] & 0xE0) == 0x00) {
                    ...
                    countWr = CO_fifo_write(
                        &SDO_C->bufFifo, &SDO_C->CANrxData[1], count, NULL);
    ...
    if (ret == CO_SDO_RT_waitingResponse) {
        ...
        if (SDO_C->state == CO_SDO_ST_UPLOAD_BLK_SUBBLOCK_SREQ) {
            ...
            if (SDO_C->block_timeoutTimer >= SDO_C->block_SDOtimeoutTime_us) {
                ...
                /* check again after timeout time elapsed */
                uint32_t diff =
                    SDO_C->block_SDOtimeoutTime_us - SDO_C->block_timeoutTimer;
                if (*timerNext_us > diff) {
                    *timerNext_us = diff;
                }
            }
