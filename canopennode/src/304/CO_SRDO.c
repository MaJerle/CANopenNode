                             ...
                             uint32_t COB_IDinverted)
{
    ...
    if (*SRDO->SRDOGuard->configurationValid == CO_SRDO_VALID_MAGIC &&
        (SRDO->SRDOCommPar->informationDirection == CO_SRDO_TX ||
         SRDO->SRDOCommPar->informationDirection == CO_SRDO_RX) &&
        SRDO->dataLength) {
...
static uint32_t CO_SRDOfindMap(CO_SDO_t *SDO,
                               uint32_t map,
                               uint8_t R_T,
                               uint8_t **ppData,
                               uint8_t *pLength,
                               ...
                               uint8_t *pIsMultibyteVar)
{
    ...
    *pLength += dataLen;
    ...
    if (*pLength > 8)
        return CO_SDO_AB_MAP_LEN; /* The number and length of the objects to be
    ...
    if (entryNo == 0xFFFF || subIndex > SDO->OD[entryNo].maxSubIndex)
        return CO_SDO_AB_NOT_EXIST; /* Object does not exist in the object
                                       dictionary. */

    attr = CO_OD_getAttribute(SDO, entryNo, subIndex);
    /* Is object Mappable for RPDO? */
    if (R_T == 0 &&
        !((attr & CO_ODA_RPDO_MAPABLE) && (attr & CO_ODA_WRITEABLE)))
        return CO_SDO_AB_NO_MAP; /* Object cannot be mapped to the PDO. */
...
static uint32_t CO_SRDOconfigMap(CO_SRDO_t *SRDO, uint8_t noOfMappedObjects)
{
    ...
    for (i = noOfMappedObjects; i > 0; i--) {
        ...
        ret = CO_SRDOfindMap(SRDO->SDO,
                             map,
                             SRDO->SRDOCommPar->informationDirection ==
                                 CO_SRDO_TX,
                             &pData,
                             length,
                             &dummy,
        ...
        if (ret) {
            *length = 0;
            CO_errorReport(
                SRDO->em, CO_EM_PDO_WRONG_MAPPING, CO_EMC_PROTOCOL_ERROR, map);
            break;
        }
...
static CO_SDO_abortCode_t CO_ODF_SRDOcom(CO_ODF_arg_t *ODF_arg)
{
    ...
    if (ODF_arg->reading) {
        if (ODF_arg->subIndex == 5 || ODF_arg->subIndex == 6) {
            ...
            if (((value)&0x7FF) == SRDO->defaultCOB_ID[index] &&
                SRDO->nodeId <= 64)
                value += SRDO->nodeId;

            /* If SRDO is not valid, set bit 31. but I do not think this applies
             * to SRDO ?! */
    ...
    if (*SRDO->SRDOGuard->operatingState == CO_NMT_OPERATIONAL)
        return CO_SDO_AB_DATA_DEV_STATE; /* Data cannot be transferred or stored
                                            to the application because of the
    ...
    if (ODF_arg->subIndex == 1) {
        ...
        if (*value != 254)
            return CO_SDO_AB_INVALID_VALUE; /* Invalid value for parameter
                                               (download only). */
         ...
         * nodeId */
        if (SRDO->nodeId <= 64 &&
            value == (SRDO->defaultCOB_ID[index] + SRDO->nodeId)) {
...
static CO_SDO_abortCode_t CO_ODF_SRDOmap(CO_ODF_arg_t *ODF_arg)
{
    ...
    if (ODF_arg->subIndex == 0) {
        ...
        if (*value > 16 || *value & 1) /*only odd numbers are allowed*/
            return CO_SDO_AB_MAP_LEN;  /* Number and length of object to be
                                          mapped exceeds SRDO length. */
    } else {
...
static CO_SDO_abortCode_t CO_ODF_SRDOcrc(CO_ODF_arg_t *ODF_arg)
{
    ...
    if (!ODF_arg->reading) {
        if (*SRDOGuard->operatingState == CO_NMT_OPERATIONAL)
            return CO_SDO_AB_DATA_DEV_STATE; /* Data cannot be transferred or
                                                stored to the application
                                                because of the present device
                                                state. */
...
{
    /* verify arguments */
    if (SRDOGuard == NULL || SDO == NULL || operatingState == NULL ||
    ...
    CO_OD_configure(
        SDO, idx_SRDOvalid, CO_ODF_SRDOvalid, (void *)SRDOGuard, 0, 0);
                              ...
                              uint16_t CANdevTxIdxInverted)
{
    ...
    if (SRDO == NULL || SRDOGuard == NULL || em == NULL || SDO == NULL ||
        checksum == NULL || SRDOCommPar == NULL || SRDOMapPar == NULL ||
                     ...
                     uint32_t *timerNext_us)
{
    ...
    if ((commands & (1 << 0)) &&
        *SRDO->SRDOGuard->configurationValid == CO_SRDO_VALID_MAGIC) {
        if (CO_SRDOconfigMap(SRDO, SRDO->SRDOMapPar->numberOfMappedObjects) ==
            0) {
            CO_SRDOconfigCom(SRDO,
                             SRDO->SRDOCommPar->COB_ID1_normal,
                             SRDO->SRDOCommPar->COB_ID2_inverted);
        } else {
            SRDO->valid = CO_SRDO_INVALID;
        }
    }
            ...
            if (SRDO->timer == 0) {
                if (SRDO->toogle) {
                    ...
                    if (SRDO->SDO->ODExtensions) {
                        ...
                        for (i = SRDO->SRDOMapPar->numberOfMappedObjects; i > 0;
                             i--) {
                            uint32_t map = *(pMap++);
                            ...
                            if (entryNo != 0xFFFF) {
                                ...
                                if (ext->pODFunc != NULL) {
                                    ...
                                    ODF_arg.data = CO_OD_getDataPointer(
                                        pSDO, entryNo, subIndex); /* https://github.com/CANopenNode/CANopenNode/issues/100
                                                                   */
                ...
                if (SRDO->toogle) {
                    ...
                    if (data_ok) {
                        ...
                        if (SRDO->SDO->ODExtensions) {
                            ...
                            for (i = SRDO->SRDOMapPar->numberOfMappedObjects;
                                 i > 0;
                                 i--) {
                                uint32_t map = *(pMap++);
                                uint16_t index = (uint16_t)(map >> 16);
                    ...
                    SRDO->timer =
                        SRDO->SRDOCommPar->safetyRelatedValidationTime * 1000U;
                }
                SRDO->toogle = !SRDO->toogle;
            }
