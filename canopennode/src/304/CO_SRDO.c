...
static void CO_SRDO_receive_normal(void *object, void *msg)
{
    ...
    if ((SRDO->valid == CO_SRDO_RX) && (DLC >= SRDO->dataLength) &&
        !CO_FLAG_READ(SRDO->CANrxNew[1])) {
...
static void CO_SRDOconfigCom(CO_SRDO_t *SRDO,
                             uint32_t COB_IDnormal,
                             uint32_t COB_IDinverted)
{
    uint16_t IDs[2][2] = {0};
    uint16_t *ID;
    ...
    /* is SRDO used? */
    if ((SRDO->SRDOGuard->configurationValid == CO_SRDO_VALID_MAGIC) &&
        ((SRDO->CommPar_informationDirection == CO_SRDO_TX) ||
         (SRDO->CommPar_informationDirection == CO_SRDO_RX)) &&
        SRDO->dataLength) {
        ...
        for (i = 0; i < 2; i++) {
            if (!(COB_ID[i] & 0xBFFFF800L)) {
                ...
                if ((ID[i] == SRDO->defaultCOB_ID[i]) &&
                    (SRDO->nodeId <= 64U)) {
                    ID[i] += (uint16_t)SRDO->nodeId * 2;
                }
                if ((0x101U <= ID[i]) && (ID[i] <= 0x180U) &&
                    ((ID[i] & 1) != i)) {
                    successCount++;
                }
            }
    ...
    for (i = 0; i < 2; i++) {
        CO_ReturnError_t r;
        SRDO->CANtxBuff[i] =
            CO_CANtxBufferInit(SRDO->CANdevTx,       /* CAN device */
                               SRDO->CANdevTxIdx[i], /* index of specific buffer
                                                        inside CAN module */
        ...
        r = CO_CANrxBufferInit(
            ...
            i ? CO_SRDO_receive_inverted
              : CO_SRDO_receive_normal); /* this function will process received
                                            message */
        if (r != CO_ERROR_NO) {
            SRDO->valid = CO_SRDO_INVALID;
...
SRDO_configMap(CO_SRDO_t *SRDO, OD_t *OD, OD_entry_t *OD_SRDOMapPar)
{
    ODR_t odRet;
    ...
    for (uint8_t i = 0; i < mappedObjectsCount; i++) {
        ...
        if (((mappedLengthBits & 0x07) != 0) ||
            (pdoDataLength[plain_inverted] > CO_SRDO_MAX_SIZE)) {
            //*erroneousMap = map;
            return CO_ERROR_NO;
        }
        ...
        if ((index < 0x20) && (subIndex == 0U)) {
            for (uint8_t j = pdoDataStart; j < pdoDataLength[plain_inverted];
                 j++) {
                static uint8_t dummyTX = 0;
                static uint8_t dummyRX;
                SRDO->mapPointer[plain_inverted][j] =
                    isRSRDO ? &dummyRX : &dummyTX;
            }
            continue;
        }
        ...
        if (odRet != ODR_OK || (OD_IO.stream.attribute & testAttribute) == 0 ||
        ...
        if ((OD_IO.stream.attribute & ODA_MB) != 0) {
            uint8_t *odDataPointer =
                OD_IO.stream.dataOrig + OD_IO.stream.dataLength - 1;
    ...
    if (pdoDataLength[0] == pdoDataLength[1]) {
        ...
        SRDO->dataLength = 0;
        CO_errorReport(
            SRDO->em, CO_EM_PDO_WRONG_MAPPING, CO_EMC_PROTOCOL_ERROR, 0);
        return CO_ERROR_OD_PARAMETERS;
    }
...
static uint16_t CO_SRDOcalcCrc(const CO_SRDO_t *SRDO)
{
    ...
    Caution: if the node id changes and you are using the default COB-ID you
    have to recalculate the checksum This behaviour is controversial and could
    be changed or made optional.
    */
...
static ODR_t OD_read_SRDO_communicationParam(OD_stream_t *stream,
                                             void *buf,
                                             OD_size_t count,
                                             OD_size_t *countRead)
{
    ...
    if ((stream->subIndex == 5U) || (stream->subIndex == 6U)) {
        ...
        if (((value & 0x7FFU) == SRDO->defaultCOB_ID[index]) &&
            (SRDO->nodeId <= 64U)) {
            value += (uint32_t)SRDO->nodeId * 2;
        }

        /* If SRDO is not valid, set bit 31. but I do not think this applies to
         * SRDO ?! */
...
static ODR_t OD_write_SRDO_communicationParam(OD_stream_t *stream,
                                              const void *buf,
                                              OD_size_t count,
                                              OD_size_t *countWritten)
{
    ...
    if (SRDOGuard->operatingState) {
        return ODR_DATA_DEV_STATE; /* Data cannot be transferred or stored to
    ...
    if (stream->subIndex == 1U) {
        ...
        uint8_t value = CO_getUint8(buf);
        if (value != 254) {
            return ODR_INVALID_VALUE; /* Invalid value for parameter (download
                                         only). */
        }
        ...
        /* check value range, the spec does not specify if COB-ID flags are
         * allowed */
        ...
        /* if default COB-ID is being written, write defaultCOB_ID without
         * nodeId */
        if ((SRDO->nodeId <= 64U) &&
            (value ==
             (SRDO->defaultCOB_ID[index] + ((uint32_t)SRDO->nodeId * 2)))) {
...
static ODR_t OD_write_SRDO_mappingParam(OD_stream_t *stream,
                                        const void *buf,
                                        OD_size_t count,
                                        OD_size_t *countWritten)
{
    ...
    if (stream->subIndex == 0U) {
        ...
        if ((value > 16U) || (value & 1)) { /*only odd numbers are allowed*/
            return ODR_MAP_LEN; /* Number and length of object to be mapped
                                   exceeds SRDO length. */
        }
...
static ODR_t OD_read_13FE(OD_stream_t *stream,
                          void *buf,
                          OD_size_t count,
                          OD_size_t *countRead)
{
...
static ODR_t OD_write_13FE(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
...
static ODR_t OD_write_13FF(OD_stream_t *stream,
                           const void *buf,
                           OD_size_t count,
                           OD_size_t *countWritten)
{
                                   ...
                                   uint32_t *errInfo)
{
    ...
    /* verify arguments */
    if ((SRDOGuard == NULL) || (OD_13FE_SRDOValid == NULL) ||
        (OD_13FF_SRDOCRC == NULL)) {
    ...
    SRDOGuard->checkCRC =
        (SRDOGuard->configurationValid == CO_SRDO_VALID_MAGIC);
...
uint8_t CO_SRDOGuard_process(CO_SRDOGuard_t *SRDOGuard, bool_t NMTisOperational)
{
    uint8_t result = 0;
                              ...
                              uint32_t *errInfo)
{
    ...
    odRet =
        OD_get_u8(OD_130x_SRDOCommPar, 3, &safetyRelatedValidationTime, true);
    if (odRet != ODR_OK) {
        if (errInfo != NULL) {
                     ...
                     uint32_t *timerNext_us)
{
    ...
    if (commands & CO_SRDO_BITCMD_CALC_CRC) {
        ...
        uint16_t crcSRDO = 0;
        odRet = OD_get_u16(
            SRDO->SRDOGuard->SRDO_CRC, SRDO->SRDO_Index + 1, &crcSRDO, true);
        if ((odRet != ODR_OK) || (crcSRDO != crcValue)) {
            SRDO->SRDOGuard->configurationValid = 0;
        }
    }

    if ((commands & CO_SRDO_BITCMD_OPERATIONAL) &&
        (SRDO->SRDOGuard->configurationValid == CO_SRDO_VALID_MAGIC)) {
        if (SRDO_configMap(SRDO, SRDO->OD, SRDO->SRDOMapPar) == CO_ERROR_NO) {
            CO_SRDOconfigCom(SRDO,
                             SRDO->CommPar_COB_ID1_normal,
                             SRDO->CommPar_COB_ID2_inverted);
        } else {
            SRDO->valid = CO_SRDO_INVALID;
        }
    }
        ...
        if (SRDO->valid == CO_SRDO_TX) {
            if (SRDO->timer == 0U) {
                if (SRDO->toogle) {
                    ...
                    SRDO->timer =
                        ((uint32_t)SRDO->CommPar_safetyCycleTime * 1000U) -
                    ...
                    if (SRDO->SDO->ODExtensions) {
                        /* for each mapped OD, check mapping to see if an OD
                         * extension is available, and call it if it is */
                        const uint32_t *pMap =
                            &SRDO->SRDOMapPar->mappedObjects[0];
                        ...
                        for (i = SRDO->mappedObjectsCount; i > 0; i--) {
                            ...
                            if (entryNo != 0xFFFF) {
                                ...
                                if (ext->pODFunc != NULL) {
                                    ...
                                    ODF_arg.attribute = CO_OD_getAttribute(
                                        pSDO, entryNo, subIndex);
                                    ODF_arg.pFlags = CO_OD_getFlagsPointer(
                                        pSDO, entryNo, subIndex);
                                    ODF_arg.data = CO_OD_getDataPointer(
                                        pSDO,
                                        entryNo,
                                        subIndex); /* https://github.com/CANopenNode/CANopenNode/issues/100
                                                    */
                                    ODF_arg.dataLength = CO_OD_getLength(
                                        pSDO, entryNo, subIndex);
                                    ext->pODFunc(&ODF_arg);
                                }
                            }
                    ...
                    if (data_ok) {
                        ...
                        for (i = 0; i < SRDO->dataLength; i++) {
                            ...
                            pSRDOdataByte_inverted[i] =
                                *(ppODdataByte_inverted[i]);
                        }
            ...
            if (timerNext_us != NULL) {
                if (*timerNext_us > SRDO->timer) {
                    *timerNext_us =
                        SRDO->timer; /* Schedule for next message timer */
                }
            }
#endif
        } else if (SRDO->valid == CO_SRDO_RX) {
            if (CO_FLAG_READ(SRDO->CANrxNew[SRDO->toogle])) {
                ...
                if (SRDO->toogle) {
                    ...
                    if (data_ok) {
                        ...
                        /* Copy data to Object dictionary. If between the copy
                         * operation CANrxNew is set to true by receive thread,
                         * then copy the latest data again. */
                    ...
                    SRDO->timer =
                        (uint32_t)SRDO->CommPar_safetyRelatedValidationTime *
                        1000U;
                }
            ...
            if (SRDO->timer == 0U) {
                SRDO->toogle = 0;
                SRDO->timer =
                    (uint32_t)SRDO->CommPar_safetyRelatedValidationTime * 1000U;
                CO_FLAG_CLEAR(SRDO->CANrxNew[0]);
                CO_FLAG_CLEAR(SRDO->CANrxNew[1]);
                /* save state */
                if (SRDO->pFunctSignalSafe != NULL) {
