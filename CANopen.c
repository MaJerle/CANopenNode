...
CO_t *CO_new(CO_config_t *config, uint32_t *heapMemoryUsed)
{
    ...
    do {
        ...
        if (CO_GET_CNT(HB_CONS) == 1) {
            ...
            CO_alloc_break_on_fail(
                co->HBcons, CO_GET_CNT(HB_CONS), sizeof(*co->HBcons));
            CO_alloc_break_on_fail(co->HBconsMonitoredNodes,
                                   countOfMonitoredNodes,
                                   sizeof(*co->HBconsMonitoredNodes));
        ...
        if (CO_GET_CNT(EM) == 1) {
            ...
            if (fifoSize >= 2) {
                CO_alloc_break_on_fail(
                    co->em_fifo, fifoSize, sizeof(*co->em_fifo));
            }
#endif
        }
        ...
        if (CO_GET_CNT(SDO_SRV) > 0) {
            CO_alloc_break_on_fail(
                co->SDOserver, CO_GET_CNT(SDO_SRV), sizeof(*co->SDOserver));
        ...
        if (CO_GET_CNT(SDO_CLI) > 0) {
            CO_alloc_break_on_fail(
                co->SDOclient, CO_GET_CNT(SDO_CLI), sizeof(*co->SDOclient));
        ...
        if (CO_GET_CNT(TIME) == 1) {
            CO_alloc_break_on_fail(
                co->TIME, CO_GET_CNT(TIME), sizeof(*co->TIME));
        ...
        if (CO_GET_CNT(SYNC) == 1) {
            CO_alloc_break_on_fail(
                co->SYNC, CO_GET_CNT(SYNC), sizeof(*co->SYNC));
        ...
        if (CO_GET_CNT(RPDO) > 0) {
            CO_alloc_break_on_fail(
                co->RPDO, CO_GET_CNT(RPDO), sizeof(*co->RPDO));
        ...
        if (CO_GET_CNT(TPDO) > 0) {
            CO_alloc_break_on_fail(
                co->TPDO, CO_GET_CNT(TPDO), sizeof(*co->TPDO));
        ...
        if (CO_GET_CNT(LEDS) == 1) {
            CO_alloc_break_on_fail(
                co->LEDs, CO_GET_CNT(LEDS), sizeof(*co->LEDs));
        }
#endif
        ...
        if (CO_GET_CNT(SRDO) > 0) {
            ...
            CO_alloc_break_on_fail(
                co->SRDO, CO_GET_CNT(SRDO), sizeof(*co->SRDO));
        ...
        if (CO_GET_CNT(LSS_SLV) == 1) {
            CO_alloc_break_on_fail(
                co->LSSslave, CO_GET_CNT(LSS_SLV), sizeof(*co->LSSslave));
        ...
        if (CO_GET_CNT(LSS_MST) == 1) {
            CO_alloc_break_on_fail(
                co->LSSmaster, CO_GET_CNT(LSS_MST), sizeof(*co->LSSmaster));
        ...
        if (CO_GET_CNT(GTWA) == 1) {
            CO_alloc_break_on_fail(
                co->gtwa, CO_GET_CNT(GTWA), sizeof(*co->gtwa));
        }
#endif
        ...
        if (CO_GET_CNT(TRACE) > 0) {
            CO_alloc_break_on_fail(
                co->trace, CO_GET_CNT(TRACE), sizeof(*co->trace));
        }
#endif
        ...
        /* CAN RX blocks */
        CO_alloc_break_on_fail(
            co->CANrx, CO_GET_CO(CNT_ALL_RX_MSGS), sizeof(*co->CANrx));

        /* CAN TX blocks */
        CO_alloc_break_on_fail(
            co->CANtx, CO_GET_CO(CNT_ALL_TX_MSGS), sizeof(*co->CANtx));
                                ...
                                uint32_t *errInfo)
{
    ...
    if (co == NULL || (CO_GET_CNT(NMT) == 0 && NMT == NULL) ||
...
CO_ReturnError_t CO_CANopenInitPDO(
    CO_t *co, CO_EM_t *em, OD_t *od, uint8_t nodeId, uint32_t *errInfo)
{
    if (co == NULL) {
        return CO_ERROR_ILLEGAL_ARGUMENT;
    }
    if (nodeId < 1 || nodeId > 127 || co->nodeIdUnconfigured) {
        return (co->nodeIdUnconfigured) ? CO_ERROR_NODE_ID_UNCONFIGURED_LSS
                              ...
                              uint32_t *timerNext_us)
{
    ...
    bool_t NMTisPreOrOperational =
        (NMTstate == CO_NMT_PRE_OPERATIONAL || NMTstate == CO_NMT_OPERATIONAL);
...
#define CO_STATUS_FIRMWARE_DOWNLOAD_IN_PROGRESS 0
    ...
    if (CO_GET_CNT(EM) == 1) {
        CO_EM_process(
            co->em, NMTisPreOrOperational, timeDifference_us, timerNext_us);
    }

    /* NMT_Heartbeat */
    if (CO_GET_CNT(NMT) == 1) {
        reset =
            CO_NMT_process(co->NMT, &NMTstate, timeDifference_us, timerNext_us);
    }
    ...
    if (CO_GET_CNT(HB_CONS) == 1) {
        CO_HBconsumer_process(
            co->HBcons, NMTisPreOrOperational, timeDifference_us, timerNext_us);
    }
#endif
    ...
    CO_nodeGuardingMaster_process(
        co->NGmaster, timeDifference_us, timerNext_us);
#endif
    ...
    if (CO_GET_CNT(GTWA) == 1) {
        CO_GTWA_process(
            co->gtwa, enableGateway, timeDifference_us, timerNext_us);
    }
#endif
...
CO_process_SYNC(CO_t *co, uint32_t timeDifference_us, uint32_t *timerNext_us)
{
    ...
    if (!co->nodeIdUnconfigured && CO_GET_CNT(SYNC) == 1) {
        ...
        CO_SYNC_status_t sync_process = CO_SYNC_process(
            co->SYNC, NMTisPreOrOperational, timeDifference_us, timerNext_us);
                     ...
                     uint32_t *timerNext_us)
{
    ...
    for (int16_t i = 0; i < CO_GET_CNT(SRDO); i++) {
        CO_SRDO_process(
            &co->SRDO[i], firstOperational, timeDifference_us, timerNext_us);
    }
}
#endif
