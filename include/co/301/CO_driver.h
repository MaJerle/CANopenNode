 ...
 */
static inline uint16_t CO_CANrxMsg_readIdent(void *rxMsg) { return 0; }
 ...
 */
static inline uint8_t CO_CANrxMsg_readDLC(void *rxMsg) { return 0; }
 ...
 */
static inline uint8_t *CO_CANrxMsg_readData(void *rxMsg) { return NULL; }
...
#define CO_IS_RESTRICTED_CAN_ID(CAN_ID)                                        \
    ((CAN_ID) <= 0x7F || ((CAN_ID) >= 0x101 && (CAN_ID) <= 0x180) ||           \
     ((CAN_ID) >= 0x581 && (CAN_ID) <= 0x5FF) ||                               \
     ((CAN_ID) >= 0x601 && (CAN_ID) <= 0x67F) ||                               \
     ((CAN_ID) >= 0x6E0 && (CAN_ID) <= 0x6FF) || (CAN_ID) >= 0x701)
#endif
