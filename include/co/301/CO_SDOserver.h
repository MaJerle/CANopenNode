...
typedef enum {
    ...
    /**
     * - SDO client: Node-ID of the SDO server is the same as node-ID of this
     * node, SDO client is the same device as SDO server. Transfer data directly
     * without communication on CAN.
     * - SDO server does not use this state. */
    CO_SDO_ST_DOWNLOAD_LOCAL_TRANSFER = 0x10U,
    /**
     * - SDO client initiates SDO download:
     *  - byte 0: @b 0010nnes binary: (nn: if e=s=1, number of data bytes, that
     * do
     *    @b not contain data; e=1 for expedited transfer; s=1 if data size is
     *    indicated.)
     *  - byte 1..3: Object index and subIndex.
     *  - byte 4..7: If e=1, expedited data are here. If e=0 s=1, size of data
     * for segmented transfer is indicated here.
     ...
     *    nnn: number of data bytes, that do @b not contain data; c=1 if this is
     * the last segment).
     ...
     * - SDO client waits for response.
     * - SDO server responses:
     *  - byte 0: @b 001t0000 binary: (t: toggle bit, set to 0 in first
     * segment).
     *  - byte 1..7: Reserved.
     ...
     * - SDO client waits for response.
     * - SDO server responses:
     *  - byte 0: @b 0100nnes binary: (nn: if e=s=1, number of data bytes, that
     * do
     *    @b not contain data; e=1 for expedited transfer; s=1 if data size is
     *    indicated).
     *  - byte 1..3: Object index and subIndex.
     ...
     *  - byte 0: @b 011t0000 binary: (t: toggle bit, set to 0 in first
     * segment).
     *  - byte 1..7: Reserved.
     ...
     *  - byte 1..3: Object index and subIndex.
     *  - byte 4..7: If s=1, then size of data for block download is indicated
     * here.
     * - SDO server is in #CO_SDO_ST_IDLE state and waits for client request. */
     ...
     *    data.)
     *  - byte 1..3: Object index and subIndex.
     *  - byte 4: blksize: Number of segments per block that shall be used by
     * the client for the following block download with 0 < blksize < 128.
     *  - byte 5..7: Reserved. */
     ...
     *    enter SDO block download end phase; nnnnnnn is sequence number of
     * segment, 1..127.
     ...
     *    server indicates the client that the segment with the sequence number
     * 1 was not received correctly and all segments shall be retransmitted by
     * the client.
     *  - byte 2: Number of segments per block that shall be used by the client
     * for the following block download with 0 < blksize < 128.
     *  - byte 3..7: Reserved.
     * - If c was set to 1, then communication enters SDO block download end
     * phase.
     */
     ...
     *    contain data)
     *  - byte 1..2: 16 bit CRC for the data set, if enabled by client and
     * server.
     *  - byte 3..7: Reserved.
     ...
     *  - byte 5: pst - protocol switch threshold. If pst > 0 and size of the
     * data in bytes is less or equal pst, then the server may switch to the SDO
     ...
     *    data; s=1 if data size is indicated. )
     *  - byte 1..3: Object index and subIndex.
     *  - byte 4..7: If s=1, then size of data for block upload is indicated
     * here.
     ...
     *    enter SDO block upload end phase; nnnnnnn is sequence number of
     * segment, 1..127.
     ...
     *    client indicates the server that the segment with the sequence number
     * 1 was not received correctly and all segments shall be retransmitted by
     * the server.
     *  - byte 2: Number of segments per block that shall be used by the server
     * for the following block upload with 0 < blksize < 128.
     *  - byte 3..7: Reserved.
     ...
     *  - byte 1..7: Reserved.
     * - SDO server waits for response.
     * - Block download successfully ends here. Note that this communication
     * ends with client response. Client may then start next SDO communication
     *   immediately.
     */
